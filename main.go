...
func main() {
	...
	boardPlus.GET("/admin/documents/:id/editable", uploads.GetEditableDocument)
	boardPlus.PUT("/admin/documents/:id/editable", uploads.SaveEditableDocument)
	boardPlus.POST("/admin/documents/:id/convert-to-word", uploads.ConvertDocumentToWord)
	...
	authed.GET("/imap/folders", imapH.ListFolders)
...
`
	...
	e.GET("/*", func(c echo.Context) error {
		req := c.Request()
		orig := req.URL.Path
		// Cache policy: content-hashed assets are immutable and cached forever;
		// everything else (the SPA shell, service worker, manifest) must never be
		// served stale, or deployed changes won't reach clients on the next load.
		switch {
		case strings.HasPrefix(orig, "/assets/"):
