...
const PLAYER_STATUS_LABELS: Record<string, string> = { active: 'Active', injury_reserve: 'Injury Reserve', vacation_hold: 'Vacation Hold', inactive: 'Inactive', suspended: 'Suspended' }
const PLAYER_STATUS_COLORS: Record<string, string> = { active: 'bg-green-100 text-green-700', injury_reserve: 'bg-red-100 text-red-600', vacation_hold: 'bg-purple-100 text-purple-600', inactive: 'bg-gray-100 text-gray-500', suspended: 'bg-red-200 text-red-800' }
const REG_STATUS_COLORS: Record<string, string> = { pending: 'bg-yellow-100 text-yellow-800', approved: 'bg-green-100 text-green-700', rejected: 'bg-red-100 text-red-600' }
const CHAL_STATUS_COLORS: Record<string, string> = { pending: 'bg-yellow-100 text-yellow-800', accepted: 'bg-blue-100 text-blue-700', completed: 'bg-green-100 text-green-700', declined: 'bg-gray-100 text-gray-500', expired: 'bg-gray-100 text-gray-400', forfeited: 'bg-orange-100 text-orange-700' }
const ACTION_LABELS: Record<string, string> = { create_ladder: 'Created Ladder', update_ladder: 'Updated Ladder', registration_approved: 'Approved Registration', registration_rejected: 'Rejected Registration', set_rank: 'Set Rank', set_player_status: 'Changed Status', enter_result: 'Entered Result', forfeit: 'Forfeited Match', reverse_result: 'Reversed Result', award_points: 'Awarded Points', conduct_warning: 'Issued Warning', conduct_suspension: 'Issued Suspension' }
...
const EMPTY_FORM = { name: '', type: 'singles', season_year: new Date().getFullYear(), status: 'draft', challenge_range: 3, challenge_expiry_days: 7, response_window_hours: 48, play_window_days: 10, challenge_frequency_days: 0, description: '' }

export default function AdminTennisChallengeLadder() {
  type Tab = 'setup' | 'registrations' | 'rankings' | 'challenges' | 'conduct' | 'audit'
  const [tab, setTab] = useState<Tab>('setup')

  const [ladders, setLadders] = useState<Ladder[]>([])
  const [activeLid, setActiveLid] = useState('')
  const [lForm, setLForm] = useState(EMPTY_FORM)
  ...
  const resetLForm = () => { setEditingL(null); setLForm(EMPTY_FORM); setLErr('') }
  const saveL = async () => {
  ...
  const deleteL = async (id: string) => {
    ...
    const remaining = ladders.filter(l => l.id !== id)
    setLadders(remaining)
    if (activeLid === id) setActiveLid(remaining[0]?.id ?? '')
  }
  ...
  const issueConductAction = async () => {
    ...
    try {
      await api.ladder.admin.issueConductAction(activeLid, { user_id: conductUserId, type: conductType, reason: conductReason, expires_at: conductExpires || undefined })
  ...
  type TabDef = { key: Tab; label: string; badge?: number }
  const tabDef: TabDef[] = [
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-xl font-bold text-gray-800">Tennis Challenge Ladder</h1>
        <p className="text-gray-500 text-sm mt-0.5">
          Manage challenge ladder configurations, seed rankings, and enter match results.
        </p>
      </div>
      ...
      <div className="flex gap-1 border-b border-gray-200 flex-wrap">
        {tabDef.map(({ key, label, badge }) => (
          <button key={key} onClick={() => setTab(key)}
            ...
            {label}
            {badge ? <span className="ml-1.5 bg-yellow-400 text-white text-xs rounded-full px-1.5 py-0.5">{badge}</span> : null}
          </button>
        ))}
      </div>
      ...
      {tab === 'setup' && (
        <div className="space-y-5">
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 space-y-4">
            <h2 className="font-semibold text-gray-700">{editingL ? 'Edit Ladder' : 'Create Challenge Ladder'}</h2>

            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <input value={lForm.name} onChange={e => setLForm(f => ({ ...f, name: e.target.value }))}
                placeholder="Ladder name (e.g. Challenge Ladder 2026)"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <input type="number" value={lForm.season_year} onChange={e => setLForm(f => ({ ...f, season_year: +e.target.value }))}
                placeholder="Season year"
                className="border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
            </div>

            <div className="border border-gray-100 rounded-lg p-4 space-y-3 bg-gray-50">
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Challenge Rules</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                <div>
                  <label className="text-xs text-gray-500 block mb-1">Challenge range</label>
                  <div className="flex items-center gap-1">
                    <input type="number" min={1} max={10} value={lForm.challenge_range}
                      onChange={e => setLForm(f => ({ ...f, challenge_range: +e.target.value }))}
                    ...
                    <span className="text-xs text-gray-400 whitespace-nowrap">spots up</span>
                  </div>
                </div>
                <div>
                  <label className="text-xs text-gray-500 block mb-1">Response window</label>
                  <div className="flex items-center gap-1">
                    <input type="number" min={1} value={lForm.response_window_hours}
                      onChange={e => setLForm(f => ({ ...f, response_window_hours: +e.target.value }))}
                      className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500 bg-white" />
                    <span className="text-xs text-gray-400">hours</span>
                  </div>
                </div>
                <div>
                  <label className="text-xs text-gray-500 block mb-1">Play window</label>
                  <div className="flex items-center gap-1">
                    <input type="number" min={1} value={lForm.play_window_days}
                      onChange={e => setLForm(f => ({ ...f, play_window_days: +e.target.value }))}
                      className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500 bg-white" />
                    <span className="text-xs text-gray-400">days</span>
                  </div>
                </div>
                <div>
                  <label className="text-xs text-gray-500 block mb-1">Frequency cap</label>
                  <div className="flex items-center gap-1">
                    <input type="number" min={0} value={lForm.challenge_frequency_days}
                      onChange={e => setLForm(f => ({ ...f, challenge_frequency_days: +e.target.value }))}
                      placeholder="0 = off"
                      className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500 bg-white" />
                    <span className="text-xs text-gray-400">days</span>
                  </div>
              ...
              <p className="text-xs text-gray-400">
                A player may challenge anyone ranked up to <strong>{lForm.challenge_range}</strong> spot{lForm.challenge_range !== 1 ? 's' : ''} above them.
                The challenged player has <strong>{lForm.response_window_hours}h</strong> to accept, and both players
                have <strong>{lForm.play_window_days} days</strong> to complete the match.
                {lForm.challenge_frequency_days > 0 && ` A player may only issue one challenge per ${lForm.challenge_frequency_days} days.`}
              </p>
            </div>

            <textarea value={lForm.description} onChange={e => setLForm(f => ({ ...f, description: e.target.value }))}
              placeholder="Description — shown to members on the ladder page (optional)" rows={2}
            ...
            <div className="flex gap-2">
              <button onClick={saveL} disabled={lSaving || !lForm.name}
                ...
                {lSaving ? 'Saving…' : editingL ? 'Update Ladder' : 'Create Ladder'}
              </button>
              {editingL && <button onClick={resetLForm} className="text-sm text-gray-500 hover:text-gray-700 px-3">Cancel</button>}
            </div>
          </div>

          {/* Existing ladders list */}
          {ladders.length > 0 && (
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
              <div className="px-4 py-3 border-b border-gray-100 bg-gray-50">
                <h3 className="text-sm font-semibold text-gray-700">Challenge Ladders</h3>
              </div>
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-50">
                  {ladders.map(l => {
                    const lEntries = l.id === activeLid ? entries : []
                    return (
                      <tr key={l.id} className={`hover:bg-gray-50 ${activeLid === l.id ? 'bg-green-50/40' : ''}`}>
                        ...
                        <td className="px-4 py-3 text-xs text-gray-400">
                          ±{l.challenge_range} · {l.response_window_hours}h · {l.play_window_days}d
                        </td>
                        <td className="px-4 py-3 text-xs text-gray-400">
                          {l.id === activeLid ? entries.length : '—'}
                        </td>
        ...
        <div className="space-y-4">
          {pendingRegs.length > 0 && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-xl overflow-hidden">
              ...
              <table className="w-full text-sm">
                <thead className="text-xs text-gray-500 uppercase border-b border-yellow-100">
                  <tr>
                    <th className="px-4 py-2 text-left">Member</th>
                    <th className="px-4 py-2 text-left">USTA</th>
                    <th className="px-4 py-2 text-left">Self-Rating</th>
                    <th className="px-4 py-2 text-left">Pref</th>
                ...
                <tbody className="divide-y divide-yellow-50">
                  {pendingRegs.map(r => (
                    <tr key={r.id} className="hover:bg-yellow-50/60">
                      <td className="px-4 py-2.5">
                        <div className="font-medium text-gray-800">{r.name}</div>
                        <div className="text-xs text-gray-400">{r.email}</div>
                      </td>
                      <td className="px-4 py-2.5 text-gray-600">{r.usta_rating || '—'}</td>
                      <td className="px-4 py-2.5 text-gray-600">{r.self_rating ?? '—'}</td>
                      <td className="px-4 py-2.5 text-gray-500 capitalize">{r.preference}</td>
                      <td className="px-4 py-2.5 text-xs text-gray-500 max-w-[130px] truncate">{r.availability || '—'}</td>
                      <td className="px-4 py-2.5 text-xs text-gray-500 max-w-[130px] truncate">{r.notes || '—'}</td>
                      <td className="px-4 py-2.5 text-xs text-gray-400">{fmtDate(r.created_at)}</td>
                      <td className="px-4 py-2.5 text-right space-x-2 whitespace-nowrap">
                        <button onClick={() => approveReg(r.user_id, 'approved')} className="text-xs font-semibold text-green-600 hover:underline">Approve</button>
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="px-4 py-3 border-b border-gray-100 bg-gray-50">
              <h3 className="text-sm font-semibold text-gray-700">All Registrations ({regs.length})</h3>
            </div>
            {regs.length === 0
              ? <p className="text-gray-400 text-sm text-center py-10">No registrations yet.</p>
              : (
                <table className="w-full text-sm">
                  <thead className="text-xs text-gray-500 uppercase border-b border-gray-100 bg-gray-50">
                    <tr>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {regs.map(r => (
                      <tr key={r.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-xs text-gray-400">{fmtDate(r.created_at)}</td>
                        <td className="px-4 py-3 text-right space-x-2 whitespace-nowrap">
                          {r.status !== 'approved' && <button onClick={() => approveReg(r.user_id, 'approved')} className="text-xs text-green-600 hover:underline font-medium">Approve</button>}
                          {r.status !== 'rejected' && <button onClick={() => approveReg(r.user_id, 'rejected')} className="text-xs text-red-500 hover:underline">Reject</button>}
                        </td>
                      </tr>
                    ))}
                  </tbody>
      ...
      {tab === 'rankings' && (
        <div className="space-y-4">
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
              ...
              <div className="flex gap-2 flex-wrap">
                <select value={rankUserId} onChange={e => setRankUserId(e.target.value)}
                  className="border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-green-500 flex-1 min-w-[150px]">
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-x-auto">
            {entries.length === 0
              ? <p className="text-gray-400 text-sm text-center py-10">No players ranked yet. Approve registrations and set ranks to build the ladder.</p>
              : (
                <table className="w-full text-sm min-w-[600px]">
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {entries.map((e, i) => (
                      <tr key={e.user_id} className={`hover:bg-gray-50 ${i === 0 ? 'bg-yellow-50/40' : ''}`}>
                        <td className="px-4 py-3">
                          <span className={`font-bold text-sm ${i === 0 ? 'text-yellow-600' : i === 1 ? 'text-gray-400' : i === 2 ? 'text-orange-400' : 'text-gray-300'}`}>
                            #{e.rank}
                          </span>
                        </td>
      ...
      {tab === 'challenges' && (
        <div className="space-y-4">
                ...
                <tbody className="divide-y divide-red-50">
                  {disputedChallenges.map(ch => (
                    <tr key={ch.id} className="hover:bg-red-50/60">
                      <td className="px-4 py-2.5 text-gray-800">#{ch.challenger_rank} {ch.challenger_name} vs #{ch.challenged_rank} {ch.challenged_name}</td>
                      <td className="px-4 py-2.5 font-mono text-sm text-gray-600">{ch.score || '—'}</td>
                      <td className="px-4 py-2.5 text-right space-x-2">
                        <button onClick={() => { setResultCid(ch.id); setResultWinner('') }} className="text-xs text-green-700 hover:underline font-medium">Enter Correct Result</button>
                ...
                <tbody className="divide-y divide-gray-50">
                  {activeChallenges.map(ch => (
                    <tr key={ch.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-2.5 text-right space-x-2 whitespace-nowrap">
                        <button onClick={() => { setResultCid(ch.id); setResultWinner('') }} className="text-xs text-green-600 hover:underline font-medium">Enter Result</button>
                        <button onClick={() => forfeit(ch.id)} className="text-xs text-orange-500 hover:underline">Forfeit</button>
                      </td>
                    </tr>
                  ))}
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-50">
                  {completedChallenges.map(ch => {
                    const winnerName = ch.winner_id === ch.challenger_id ? ch.challenger_name : ch.winner_id === ch.challenged_id ? ch.challenged_name : '—'
                    return (
                      <tr key={ch.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-2.5 font-medium text-gray-800">{winnerName}</td>
                        <td className="px-4 py-2.5 font-mono text-xs text-gray-500">{ch.score || '—'}</td>
                        <td className="px-4 py-2.5">
                          <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${CHAL_STATUS_COLORS[ch.status]}`}>{ch.status}</span>
                        </td>
                        <td className="px-4 py-2.5 text-xs text-gray-400">{fmtDate(ch.created_at)}</td>
          ...
          {challenges.length === 0 && !activeLid && (
            <p className="text-center text-gray-400 py-10 text-sm">Create a ladder in Setup to get started.</p>
          )}
          {challenges.length === 0 && activeLid && (
            <p className="text-center text-gray-400 py-10 text-sm">No challenges yet for {activeLadder?.name}.</p>
          )}
        </div>
      )}
      ...
      {tab === 'conduct' && (
        <div className="space-y-5">
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 space-y-3">
            ...
            <p className="text-xs text-gray-400">Conduct actions are visible to admins only and are included in the audit log.</p>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <select value={conductUserId} onChange={e => setConductUserId(e.target.value)}
                className="border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-red-400">
                <option value="">— Select player —</option>
                {entries.map(e => <option key={e.user_id} value={e.user_id}>{e.name} (#{e.rank})</option>)}
              </select>
              ...
              <textarea value={conductReason} onChange={e => setConductReason(e.target.value)}
                ...
                className="sm:col-span-3 border border-gray-200 rounded-lg px-3 py-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-red-400" />
            </div>
            ...
            <button onClick={issueConductAction} disabled={conductSaving || !conductUserId || !conductReason}
              ...
              {conductSaving ? 'Saving…' : `Issue ${conductType === 'warning' ? 'Warning' : 'Suspension'}`}
            </button>
          </div>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {conductRecords.map(r => (
                      <tr key={r.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-gray-600 text-sm max-w-[240px]">{r.reason}</td>
                ...
                <tbody className="divide-y divide-gray-50">
                  {auditLog.map(r => (
                    <tr key={r.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-2.5">
                        <span className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded">{ACTION_LABELS[r.action] ?? r.action}</span>
                      </td>
      ...
      {!activeLid && tab !== 'setup' && (
        <div className="text-center text-gray-400 py-12 text-sm">
          No challenge ladder created yet.{' '}
          <button onClick={() => setTab('setup')} className="text-green-600 hover:underline font-medium">Go to Setup</button>
          {' '}to create one.
        </div>
      )}
    </div>
