-- Ensure the collaborative-documents tables exist and the board-meeting proposal
-- is seeded. Migration 105 created these, but on the production database its seed
-- INSERT did not take effect (the document never appeared at /docs), and an
-- already-applied migration won't re-run. This migration is fully self-contained
-- and idempotent: CREATE ... IF NOT EXISTS plus a fixed-id INSERT ... ON CONFLICT
-- DO NOTHING, so it safely creates anything missing and never overwrites edits
-- members have made to the document.
