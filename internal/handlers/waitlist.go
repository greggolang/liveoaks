...
func (h *WaitlistHandler) Join(c echo.Context) error {
	...
	if err := c.Bind(&req); err != nil || req.Email == "" || req.FirstName == "" {
	...
	if err != nil {
		return echo.NewHTTPError(http.StatusConflict, "email already on waitlist")
	}
...
func (h *WaitlistHandler) List(c echo.Context) error {
	...
	for rows.Next() {
		var w WaitlistEntry
		if err := rows.Scan(&w.ID, &w.FirstName, &w.LastName, &w.Email, &w.Phone, &w.Notes, &w.Status, &w.CreatedAt); err != nil {
			continue
		}
